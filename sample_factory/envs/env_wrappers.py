...
class RecordingWrapper(gym.core.Wrapper):
    def __init__(self, env, record_to, record_fps, player_id, random_action, input_res_w, input_res_h):
        super().__init__(env)
        ...
        self._input_res_w = input_res_w
        self._input_res_h = input_res_h
    ...
    def reset(self):
        if self._episode_recording_dir is not None and self._record_id > 0:
            if not os.path.isdir(self._episode_recording_dir):
                os.mkdir(self._episode_recording_dir)
            ...
            with open(join(self._episode_recording_dir, 'rewards.json'), 'w') as rewards_file:
                json.dump(self._recorded_rewards, rewards_file)
            ...
            def write_video(image_dir, fps):
                img_array = []
                for filename in glob.glob(join(image_dir, '*.png')):
                    img = cv2.imread(filename)
                    height, width, layers = img.shape
                    size = (width, height)
                    img_array.append(img)
                if len(img_array) > 0:
                    out = cv2.VideoWriter(image_dir + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, size)
                    for i in range(len(img_array)):
                        out.write(img_array[i])
                    out.release()

            write_video(new_dir_name, fps=self._record_fps)
        ...
        level_name = f'{self.task_id:02d}_{self.level_name}'
        dir_name = join(level_name, dir_name)

        self._episode_recording_dir = join(self._record_to, dir_name)
        self._episode_recording_dir_cells = join(self._record_to, dir_name, "cells")
        ensure_dir_exists(self._episode_recording_dir)
        ensure_dir_exists(self._episode_recording_dir_cells)
    ...
    def _record(self, observation):
        #import pickle
        #fname = join(self._episode_recording_dir, f'{self._frame_id:05d}.pkl')
        #with open(fname, 'wb') as f:
        #    pickle.dump(self.env.gen_obs()['image'], f)
        frame_name = f'{self._frame_id:05d}.png'
        if isinstance(observation, dict) and 'obs' in observation.keys():
            if observation['obs'].shape[0] == 3:
                pass
            else:
                if (self.observation_space['obs'].shape[1] != self.height) or (
                        self.observation_space['obs'].shape[2] != self.width):
                    img = cv2.cvtColor(observation['original_obs'], cv2.COLOR_RGB2BGR)
                else:
                    img = cv2.cvtColor(observation['obs'], cv2.COLOR_RGB2BGR)
                if 'INSTR' in observation.keys() and self.raw_instr != '':
                    img = cv2.resize(img, [960, 720], interpolation=cv2.INTER_CUBIC)
                    instr = self.raw_instr
                    if len(instr) > 44:
                        cv2.putText(img, f'{instr[:22]}', (10, 70), 4, 2.4, (0, 0, 0), thickness=8)
                        cv2.putText(img, f'{instr[22:]}', (10, 150), 4, 2.4, (0, 0, 0), thickness=8)
                        cv2.putText(img, f'{instr[44:]}', (10, 230), 4, 2.4, (0, 0, 0), thickness=8)
                    elif len(instr) > 22:
                        cv2.putText(img, f'{instr[:22]}', (10, 70), 4, 2.4, (0, 0, 0), thickness=8)
                        cv2.putText(img, f'{instr[22:]}', (10, 150), 4, 2.4, (0, 0, 0), thickness=8)
                    else:
                        cv2.putText(img, f'{instr}', (10, 90), 4, 2.6, (0, 0, 0), thickness=8)
                cv2.imwrite(join(self._episode_recording_dir, frame_name), img)
                if 'cell_keys' in self._recorded_data:
                    cell_dir = self._recorded_data['cell_keys'][-1]
                    cell_dir_path = join(self._episode_recording_dir_cells, cell_dir)
                    ensure_dir_exists(cell_dir_path)
                    cv2.imwrite(join(cell_dir_path, frame_name), img)
                self._frame_id += 1
        else:
    ...
    def add_data(self, data_dict):
        for key, val in data_dict.items():
            if not key in self._recorded_data:
                self._recorded_data[key] = []
            if type(val) == list and len(val) == 1:
                val = val[0]
            self._recorded_data[key].append(val)
