...
EXTENDED_ACTION_SET_SIMPLE = (
    (-10, 0, 0, 0, 0, 0, 0),  # Small Look Left
    (10, 0, 0, 0, 0, 0, 0),   # Small Look Right
    (0, 10, 0, 0, 0, 0, 0),   # Small Look Down
    (0, -10, 0, 0, 0, 0, 0),  # Small Look Up
    (0, 0, 0, 0, 0, 0, 0),    # No-op
)
...
def string_to_hash_bucket(s, vocabulary_size):
    ...
    return (int(hashlib.md5(s.encode('utf-8')).hexdigest(), 16) % (vocabulary_size - 1)) + 1 # 0 means absence of an instruction


class DmlabGymEnv(gym.Env):
    def __init__(
            self, cfg, task_id, level, action_repeat, res_w, res_h, benchmark_mode, renderer, dataset_path,
            with_instructions, extended_action_set, simple_action, use_level_cache, level_cache_path,
            gpu_index, input_res_w, input_res_h, fixed_reset_seed, extra_cfg=None
    ):
        self.cfg = cfg
        self.width = res_w
        self.height = res_h
        ...
        if self.cfg.dmlab_extended_action_set:
            self.action_set = EXTENDED_ACTION_SET
        elif self.cfg.dmlab_simple_action_set:
            self.action_set = EXTENDED_ACTION_SET_SIMPLE
        elif self.cfg.dmlab_large_action_set:
        ...
        self.observation_space = gym.spaces.Dict(
            ...
            obs=gym.spaces.Box(low=0, high=255, shape=(input_res_h, input_res_w, 3), dtype=np.uint8)
        )
        if (input_res_h != self.height) or (input_res_w != self.width):
            self.observation_space.spaces['original_obs'] = gym.spaces.Box(low=0, high=255, shape=(self.height, self.width, 3), dtype=np.uint8)
        if self.with_instructions:
    ...
    def format_obs_dict(self, env_obs_dict):
        ...
        if (self.observation_space['obs'].shape[1] != self.height) or (self.observation_space['obs'].shape[2] != self.width):
            resized_obs = cv2.resize(env_obs_dict['obs'], (self.observation_space['obs'].shape[2], self.observation_space['obs'].shape[1]), interpolation=cv2.INTER_CUBIC)
            env_obs_dict['original_obs'] = env_obs_dict['obs']
            env_obs_dict['obs'] = resized_obs

        instr = env_obs_dict.get(self.instructions_observation)
        self.raw_instr = instr
        self.instructions[:] = 0
    ...
    def step(self, action):
        ...
        self.episode_step += 1
        self.actor_env_step += 1
        info = {'num_frames': self.action_repeat,
            'prev_goal_episode_step': self.prev_goal_episode_step}
        if reward > 0:
            self.prev_goal_episode_step = self.episode_step
