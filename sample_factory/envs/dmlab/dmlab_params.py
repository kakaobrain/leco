...
def add_dmlab_env_args(env, parser):
    ...
    p.add_argument('--dmlab30_dataset', default='/path/to/your/dmlab/dataset/brady_konkle_oliva2008', type=str, help='Path to dataset needed for some of the environments in DMLab-30')
    ...
    p.add_argument('--dmlab_simple_action_set', default=False, type=str2bool, help='Use simpler, modified action set from extended action set (e.g. PopART and R2D2)')
    p.add_argument('--dmlab_large_action_set', default=False, type=str2bool,
                   help='Large up / down are appended to extended action')
    ...
    p.add_argument('--dmlab_level_cache_path', default='/path/to/you/dmlab/cache/dmlab_cache', type=str, help='Location to store the cached levels (or path to pre-generated cache)')
    p.add_argument(
    ...
    p.add_argument('--dmlab_instr_encoder_device', default='cuda', type=str, choices=['cpu', 'cuda', 'gpu'], help='Type of device for dmlab instruction encoder (cpu vs gpu(cuda)). cuda or gpu for distributed training')
    p.add_argument('--random_action', default=False, type=str2bool, help='Whether to use random action for rendering')
    p.add_argument('--dmlab_reward_shaping_div', default=5.0, type=float, help='Reward shaping division in tanh')
    p.add_argument('--dmlab_reward_shaping_neg', default=1.5, type=float, help='Reward shaping scaler negative value')
    p.add_argument('--dmlab_reward_shaping_pos', default=5.0, type=float, help='Reward shaping scaler positive value')
    p.add_argument('--time_penalty', default=False, type=str2bool, help='Whether to use time penalty')
    p.add_argument('--max_step', default=15.0, type=float, help='number of steps between consecutive pos rewards for penalizaing pos reward to 1 -> 0.1')
    p.add_argument('--time_limit', default=-1, type=float, help='Adjusting time limit to a env')
