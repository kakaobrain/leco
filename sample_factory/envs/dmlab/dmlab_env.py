...
from sample_factory.algorithms.utils.algo_utils import EXTRA_EPISODIC_STATS_PROCESSING, EXTRA_PER_POLICY_SUMMARIES, EXTRA_EPISODIC_TEST_STATS_PROCESSING, EXTRA_TEST_SUMMARIES
from sample_factory.envs.dmlab.dmlab30 import dmlab30_level_name_to_level, \
    DMLAB30_LEVELS_THAT_USE_LEVEL_CACHE, DMLAB30_LEVELS, DMLAB30_LEVELS_TEST, HUMAN_SCORES, RANDOM_SCORES, LEVEL_MAPPING
...
DMLAB_ENVS = [
    ...
    DmLabSpec('dmlab_lasertag_skymaze_nonmatch', ['contributed/dmlab30/lasertag_one_opponent_large',
    ...
    DmLabSpec('dmlab_30_test', [dmlab30_level_name_to_level(l) for l in DMLAB30_LEVELS_TEST]),
    ...
    DmLabSpec('dmlab_subset', ['contributed/dmlab30/rooms_collect_good_objects_train',
    ...
    DmLabSpec('dmlab_subset_test', ['contributed/dmlab30/rooms_collect_good_objects_test',
                               'contributed/dmlab30/language_answer_quantitative_question',
                               'contributed/dmlab30/lasertag_three_opponents_large',
                               'contributed/dmlab30/natlab_varying_map_regrowth',
                               'contributed/dmlab30/skymaze_irreversible_path_hard',
                               'contributed/dmlab30/psychlab_sequential_comparison',
                               'contributed/dmlab30/explore_object_locations_large']),
    DmLabSpec('dmlab_lasertag', ['contributed/dmlab30/lasertag_one_opponent_small',
                               'contributed/dmlab30/lasertag_one_opponent_large',
                               'contributed/dmlab30/lasertag_three_opponents_small',
                               'contributed/dmlab30/lasertag_three_opponents_large']),
    DmLabSpec('dmlab_subset_psychlab', ['contributed/dmlab30/psychlab_arbitrary_visuomotor_mapping',
...
def dmlab_env_by_name(name):
    ...
    if level.split('/')[-1] in DMLAB30_LEVELS:
        log.warning('Interpreting the level name as an single task (%s) of DMLab-30 envs!', level.split('/')[-1])
        spec = DmLabSpec(name, level)
    else:
        group = level.split('_')[0]
        log.warning('Interpreting the level name as an group (%s) of DMLab-30 envs!', group)
        spec = DmLabSpec(name, [dmlab30_level_name_to_level(l) for l in DMLAB30_LEVELS if l.startswith(group)])
...
def make_dmlab_env_impl(spec, cfg, env_config, **kwargs):
    ...
    if 'record_to' in cfg and cfg.record_to is not None:
        res_w = cfg.record_res_w
        res_h = cfg.record_res_h
        input_res_w = cfg.res_w
        input_res_h = cfg.res_h
    else:
        res_w = input_res_w = cfg.res_w
        res_h = input_res_h = cfg.res_h

    env = DmlabGymEnv(
        cfg, task_id, level, skip_frames, res_w, res_h, cfg.dmlab_throughput_benchmark, cfg.dmlab_renderer,
        get_dataset_path(cfg), cfg.dmlab_with_instructions, cfg.dmlab_extended_action_set, cfg.dmlab_simple_action_set,
        cfg.dmlab_use_level_cache, cfg.dmlab_level_cache_path,
        gpu_idx, input_res_w, input_res_h, cfg.fixed_reset_seed, spec.extra_cfg,
    )
    ...
    if 'record_to' in cfg and cfg.record_to is not None:
        env = RecordingWrapper(env, cfg.record_to, cfg.record_fps, 0, cfg.random_action, input_res_w, input_res_h)

    if cfg.pixel_format == 'CHW':
        env = PixelFormatChwWrapper(env)

    env = DmlabRewardShapingWrapper(env, cfg.dmlab_reward_shaping_div, cfg.dmlab_reward_shaping_neg, cfg.dmlab_reward_shaping_pos)
    return env
...
def make_dmlab_env(env_name, cfg=None, **kwargs):
    ...
    spec = dmlab_env_by_name(env_name)
    if cfg is not None and cfg.is_test:
        spec.extra_cfg = dict(allowHoldOutLevels='true')
    if type(spec.level) == list:
        num_envs = len(spec.level)
    else:
        num_envs = 1
...
def dmlab_extra_summaries(policy_id, policy_avg_stats, env_steps, summary_writer, cfg):
    ...
    for level_idx, level in enumerate(all_levels):
        ...
        level_ = level.replace('test', 'train')
        test_level_name = LEVEL_MAPPING[level_]
    ...
    median_normalized_score = np.median(level_mean_scores_normalized)
    ...
    summary_writer.add_scalar(f'_dmlab/000_median_human_norm_score', median_normalized_score, env_steps)
...
@static_vars(new_level_returns=dict(), env_spec=None)
def dmlab_extra_test_stats_processing(policy_id, stat_key, stat_value, cfg):
    if RAW_SCORE_SUMMARY_KEY_SUFFIX not in stat_key:
        return

    new_level_returns = dmlab_extra_test_stats_processing.new_level_returns
    if policy_id not in new_level_returns:
        new_level_returns[policy_id] = dict()

    if dmlab_extra_test_stats_processing.env_spec is None:
        dmlab_extra_test_stats_processing.env_spec = dmlab_env_by_name(cfg.env)

    task_id = int(stat_key.split('_')[1])  # this is a bit hacky but should do the job
    level = task_id_to_level(task_id, dmlab_extra_test_stats_processing.env_spec)
    level_name = dmlab_level_to_level_name(level)
...
@static_vars(all_levels=None)
def dmlab_test_summaries(policy_id, policy_avg_stats, env_steps, summary_writer, cfg):
    """
    We precisely follow IMPALA repo (scalable_agent) here for the reward calculation.
    ...
    # exit if we don't have at least one episode for all levels
    if dmlab_test_summaries.all_levels is None:
        dmlab_levels = list_all_levels_for_experiment(cfg.env)
        level_names = [dmlab_level_to_level_name(l) for l in dmlab_levels]
        dmlab_test_summaries.all_levels = level_names

    all_levels = dmlab_test_summaries.all_levels
    for level in all_levels:
        if len(new_level_returns[policy_id].get(level, [])) < 1:
            return

    level_mean_scores_normalized = []
    level_std_scores_normalized = []
    level_mean_scores_normalized_capped = []
    level_std_scores_normalized_capped = []
    ...
    for level_idx, level in enumerate(all_levels):
        ...
        human_normalized_score = (level_score - random) / (human - random) * 100
        human_normalized_score_std = np.std(human_normalized_score)
        capped_human_normalized_score = np.minimum(100.0, human_normalized_score)
        capped_human_normalized_score_std = np.std(capped_human_normalized_score)

        level_std_scores_normalized.append(human_normalized_score_std)
        level_std_scores_normalized_capped.append(capped_human_normalized_score_std)
        ...
        level_key = f'{level_idx:02d}_{level}'
        summary_writer.add_scalar(f'_dmlab/{level_key}_human_norm_score', human_normalized_score, env_steps)
        summary_writer.add_scalar(f'_dmlab/zz_{level_key}_human_norm_score_std', human_normalized_score_std, env_steps)
        summary_writer.add_scalar(f'_dmlab/capped_{level_key}_human_norm_score', capped_human_normalized_score, env_steps)
        summary_writer.add_scalar(f'_dmlab/zz_capped_{level_key}_human_norm_score_std', capped_human_normalized_score_std, env_steps)
        log.debug('Policy %d %s human_norm_score: %f', cfg.test_policy_id[policy_id], level, human_normalized_score)
        log.debug('Policy %d %s capped_human_norm_score: %f', cfg.test_policy_id[policy_id], level, capped_human_normalized_score)

    assert len(level_mean_scores_normalized) == len(level_mean_scores_normalized_capped) == len(all_levels)

    mean_normalized_score = np.mean(level_mean_scores_normalized)
    std_normalized_score = np.std(level_std_scores_normalized)
    capped_mean_normalized_score = np.mean(level_mean_scores_normalized_capped)
    capped_std_normalized_score = np.mean(level_std_scores_normalized_capped)

    # use 000 here to put these summaries on top in tensorboard (it sorts by ASCII)
    summary_writer.add_scalar(f'_dmlab/000_mean_human_norm_score', mean_normalized_score, env_steps)
    summary_writer.add_scalar(f'_dmlab/000_mean_human_norm_score_std', std_normalized_score, env_steps)
    summary_writer.add_scalar(f'_dmlab/000_capped_mean_human_norm_score', capped_mean_normalized_score, env_steps)
    summary_writer.add_scalar(f'_dmlab/000_capped_mean_human_norm_score_std', capped_std_normalized_score, env_steps)
    log.debug('Policy %d mean_human_norm_score: %f', cfg.test_policy_id[policy_id], mean_normalized_score)
    log.debug('Policy %d capped_mean_human_norm_score: %f', cfg.test_policy_id[policy_id], capped_mean_normalized_score)
    return policy_id
...
def ensure_initialized(cfg, env_name):
    ...
    if 'dmlab_' in env_name:
    #if env_name == 'dmlab_30' or 'dmlab_subset':
        ...
        if cfg.is_test:
            EXTRA_EPISODIC_TEST_STATS_PROCESSING.append(dmlab_extra_test_stats_processing)
            EXTRA_TEST_SUMMARIES.append(dmlab_test_summaries)
