...
def set_process_cpu_affinity(worker_idx, num_workers, local_rank=0, nproc_per_node=0):
    ...
    if nproc_per_node > 1:
        worker_idx_dist = worker_idx * nproc_per_node + local_rank
        num_workers_dist = num_workers * nproc_per_node
    else:
        worker_idx_dist = worker_idx
        num_workers_dist = num_workers

    core_indices = cores_for_worker_process(worker_idx_dist, num_workers_dist, cpu_count)
...
def summaries_dir(experiment_dir_, cfg):
    if cfg.is_test:
        experiment_dir_ = join(experiment_dir_, cfg.test_dir)
    return ensure_dir_exists(join(experiment_dir_, '.summary'))

def records_dir(experiment_dir_, cfg):
    return ensure_dir_exists(join(experiment_dir_, cfg.record_to))

def cfg_file(cfg, is_test=False):
    if is_test:
        params_file = join(experiment_dir(cfg=cfg), 'test_cfg.json')
    else:
...
def use_lirpg(cfg):
    return cfg.use_intrinsic and ('lirpg' in cfg.int_type or 'lirpg' in cfg.cell_learning)