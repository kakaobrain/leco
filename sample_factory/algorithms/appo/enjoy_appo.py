...
def enjoy(cfg, max_num_frames=1e9):
    ...
    episode_rewards = [deque([], maxlen=10) for _ in range(env.num_agents)]
    true_rewards = [deque([], maxlen=10) for _ in range(env.num_agents)]
    num_frames = 0
    ...
    with torch.no_grad():
        while not max_frames_reached(num_frames):
            ...
            policy_outputs = actor_critic(obs_torch, actions, torch.Tensor([raw_rew[0]]).to(device), rnn_states, 0, with_action_distribution=True)
            ...
            for _ in range(1):
                ...
                for agent_i, done_flag in enumerate(done):
                    if done_flag:
                        ...
                        log.info('Episode finished for agent %d at %d frames. Reward: %.3f, true_reward: %.3f, max_reward: %.3f', agent_i, num_frames, episode_reward[agent_i], true_rewards[agent_i][-1])
...
def main():
    ...
    status, avg_reward = enjoy(cfg=cfg)
    return status
