...
class ActorState:
    ...
    def __init__(
        ...
        if not self.cfg.use_transformer:
            ...
            self.last_int_rnn_state = None

        self.prev_actions = None
        self.prev_rewards = None
        self.prev_raw_rewards = None
    ...
    def _reset_rnn_state(self):
        ...
        if self.last_int_rnn_state is not None:
            self.last_int_rnn_state.fill_(0.0)
    ...
    def curr_output_of(self, policy_output_name):
        assert policy_output_name in self.policy_output_names
        index = sum(self.policy_output_sizes[:self.policy_output_names.index(policy_output_name)])
        size = self.policy_output_sizes[self.policy_output_names.index(policy_output_name)]
        output = self.policy_output_tensors[index:index+size]
        if output.shape[0] > 1:
    ...
    def record_env_step(self, new_ob, reward, done, info, raw_reward, traj_buffer_idx, rollout_step):
        """
        ...
        self.traj_tensors['raw_rewards'][traj_buffer_idx, rollout_step][0] = float(raw_reward)
        self.traj_tensors['episode_id'][traj_buffer_idx, rollout_step][0] = self.episode_id
        self.traj_tensors['episode_step'][traj_buffer_idx, rollout_step][0] = self.episode_step
        self.traj_tensors.set_data((traj_buffer_idx, rollout_step), dict(next_obs=new_ob))
    ...
    def finalize_trajectory(self, rollout_step, first_rollout=None):
        ...
        if self.cfg.use_transformer:
            traj_dict = dict(
                t_id=t_id, length=self.cfg.rollout, env_steps=self.rollout_env_steps,
                policy_id=self.curr_policy_id, rollout_step=rollout_step, first_rollout=first_rollout, actor_env_step=self.actor_env_step
            )
        else:
            traj_dict = dict(
                t_id=t_id, length=self.cfg.rollout, env_steps=self.rollout_env_steps,
    ...
    def episodic_stats(self):
        ...
        stats['episode_id'] = self.episode_id
...
class VectorEnvRunner:
    ...
    def __init__(self, cfg, num_envs, worker_idx, split_idx, num_agents, shared_buffers, pbt_reward_shaping):
        ...
        if self.cfg.nproc_per_node > 1:
            self.worker_idx_dist = self.worker_idx * self.cfg.nproc_per_node + self.cfg.local_rank
        else:
            self.worker_idx_dist = self.worker_idx
        ...
        if self.cfg.use_transformer:
            self.first_rollout = [True] * shared_buffers.num_traj_buffers
        ...
        self.task_id = shared_buffers.task_ids[index]
    ...
    def init(self):
        ...
        for env_i in range(self.num_envs):
            ...
            env_id = self.worker_idx_dist * self.cfg.num_envs_per_worker + vector_idx

            env_config = AttrDict(
                worker_index=self.worker_idx_dist, vector_index=vector_idx, env_id=env_id,
            )
        ...
        for env_i in range(len(self.envs)):
            for agent_i in range(self.num_agents):
                ...
                if actor_policy == policy_id:
                    ...
                    if not self.cfg.use_transformer:
                        new_rnn_state = None
                        new_int_rnn_state = None
                    for tensor_idx, name in enumerate(actor_state.policy_output_names):
                        if name == 'rnn_states' and not self.cfg.use_transformer:
                            new_rnn_state = policy_outputs[tensor_idx]
                        elif name == 'int_rnn_states' and not self.cfg.use_transformer:
                            new_int_rnn_state = policy_outputs[tensor_idx]
                        else:
                    ...
                    if not self.cfg.use_transformer:
                        ...
                        actor_state.last_int_rnn_state = new_int_rnn_state
    ...
    def _process_rewards(self, rewards, true_rewards, env_i):
        """
        ...
        for agent_i, r in enumerate(true_rewards):
            self.actor_states[env_i][agent_i].last_episode_true_reward += r
    ...
    def _process_env_step(self, new_obs, rewards, dones, infos, raw_rewards, env_i):
        """
        ...
        rewards = self._process_rewards(rewards, raw_rewards, env_i)

        for agent_i in range(self.num_agents):
            actor_state = env_actor_states[agent_i]

            actor_state.record_env_step(
                new_obs[agent_i], rewards[agent_i], dones[agent_i], infos[agent_i], raw_rewards[agent_i], self.traj_buffer_idx, self.rollout_step,
            )

            actor_state.last_obs = new_obs[agent_i]
            actor_state.prev_rewards = float(rewards[agent_i])
            actor_state.prev_raw_rewards = float(raw_rewards[agent_i])
            ...
            # if we are at the episode boundary, initialize prev_actions, prev_rewards, prev_raw_rewards
            if dones[agent_i]:
                actor_state.prev_rewards = 0.
                actor_state.prev_raw_rewards = 0.
                if 'atari' in self.cfg.env:
                    actor_state.prev_actions = torch.Tensor([0]) # 0: NOOP
                else:
    ...
    def _format_policy_request(self):
        ...
        for env_i in range(self.num_envs):
            for agent_i in range(self.num_agents):
                ...
                if self.cfg.use_transformer:
                    actor_env_step = actor_state.actor_env_step
                    data = (
                    env_i, agent_i, self.traj_buffer_idx, self.rollout_step, self.first_rollout[self.traj_buffer_idx],
                    actor_env_step)
                else:
        ...
        for env_i in range(self.num_envs):
            for agent_i in range(self.num_agents):
                ...
                if self.cfg.use_transformer:
                    ...
                    if actor_state.last_int_rnn_state is not None:
                        policy_inputs.update(dict(int_rnn_states=actor_state.last_int_rnn_state))
                ...
                if self.cfg.extended_input:
                    ...
                    actor_state.traj_tensors['prev_raw_rewards'][self.traj_buffer_idx, self.rollout_step].fill_(
                        actor_state.prev_raw_rewards)

                    # actor_state.traj_tensors['prev_actions'][self.traj_buffer_idx, self.rollout_step][0] = actor_state.prev_actions
                    # actor_state.traj_tensors['prev_rewards'][self.traj_buffer_idx, self.rollout_step][0] = actor_state.prev_rewards
                    # actor_state.traj_tensors['prev_raw_rewards'][self.traj_buffer_idx, self.rollout_step][0] = actor_state.prev_raw_rewards
        ...
        for env_i, e in enumerate(self.envs):
            ...
            if self.cfg.decorrelate_envs_on_one_worker:
                ...
                for decorrelate_step in range(decorrelate_steps):
                    ...
                    observations, rew, dones, info, raw_rew = e.step(actions)
            ...
            for agent_i, obs in enumerate(observations):
                ...
                if 'atari' in self.cfg.env:
                    actor_state.traj_tensors['prev_actions'][self.traj_buffer_idx, self.rollout_step][0] = torch.Tensor([0]) # 0: NOOP
                else:
                    actor_state.traj_tensors['prev_actions'][self.traj_buffer_idx, self.rollout_step][0] = torch.Tensor([-1]) # all zero one_hot vector

                actor_state.traj_tensors['prev_rewards'][self.traj_buffer_idx, self.rollout_step][0] = 0.
                actor_state.traj_tensors['prev_raw_rewards'][self.traj_buffer_idx, self.rollout_step][0] = 0.
                actor_state.traj_tensors['dones'][self.traj_buffer_idx, self.rollout_step][0] = torch.BoolTensor([False])
    ...
    def advance_rollouts(self, data, timing):
        ...
        the next env step.
        """
        timing.wait_buffer = 0

        with timing.timeit('save_policy_outputs'), timing.add_time('total_save_policy_outputs'):
        ...
        def record_int_data(env, data_dict):
            recorder = None
            try:
                _env = env
                while True:
                    if isinstance(_env, RecordingWrapper):
                        recorder = _env
                        break
                    else:
                        _env = _env.env
            ...
            recorder.add_data(data_dict)

        do_record_cell = self.cfg.use_intrinsic and self.cfg.int_type == 'cell' and self.cfg.is_test

        # if do_record_cell:
        #     from sample_factory.algorithms.appo.modules.cell import CellSpec
        #     cell_spec = CellSpec(self.cfg.cell_spec, type=self.cfg.cell_type)
        #     resolution

        for env_i, e in enumerate(self.envs):
            with timing.timeit('_env_step'), timing.add_time('total_env_step'):
                actions = [s.curr_actions() for s in self.actor_states[env_i]]
                if do_record_cell:
                    data_dict = {}
                    data_dict['cells'] = [np.array(s.curr_output_of('cell_ids')).astype(np.int32).tolist() for s in self.actor_states[env_i]]
                    data_dict['cell_keys'] = ['_'.join(map(str, cell)) for cell in data_dict['cells']]
                    record_int_data(e, data_dict)
                new_obs, rewards, dones, infos, raw_rewards = e.step(actions)

            timing['env_step'] += timing['_env_step']

            with timing.timeit('_overhead'), timing.add_time('total_overhead'):
                stats = self._process_env_step(new_obs, rewards, dones, infos, raw_rewards, env_i)
        ...
        if self.rollout_step == self.cfg.rollout:
            ...
            if self.traj_tensors_available[:, :, self.traj_buffer_idx].min() == 0:
                with timing.timeit('wait_buffer'), timing.add_time('total_wait_buffer'):
                    self.wait_for_traj_buffers()

        with timing.timeit('prepare_next_step'), timing.add_time('total_prepare_next_step'):
...
class ActorWorker:
    ...
    def _init(self):
        ...
        if self.cfg.set_workers_cpu_affinity:
            set_process_cpu_affinity(self.worker_idx, self.cfg.num_workers, self.cfg.local_rank, self.cfg.nproc_per_node)
    ...
    def _enqueue_complete_rollouts(self, split_idx, complete_rollouts):
        ...
        # learner isn't used when testing
        ...
        for rollout in rollouts:
            ...
            if self.cfg.use_transformer:
                ...
                first_rollout = rollout['first_rollout']
                actor_env_step = rollout['actor_env_step']
            if policy_id not in rollouts_per_policy:
                if self.cfg.use_transformer:
                    rollouts_per_policy[policy_id] = dict(
                        ...
                        rollout_step=rollout_step, first_rollout=first_rollout, actor_env_step=actor_env_step
                    )
                else:
                    rollouts_per_policy[policy_id] = dict(
                        rollouts=[], worker_idx=self.worker_idx,
            ...
            while not self.terminate:
                try:
                    ...
                    if time.time() - last_report > 5.0 and 'one_step' in timing:
                        #timing_stats = dict(wait_actor=timing.wait_actor, step_actor=timing.one_step, wait_buffer=timing.wait_buffer)
                        timing_stats = {
                            key: val for key, val in timing.items()
                            if key in ['wait_actor', 'env_step', 'overhead', 'one_step', 'save_policy_outputs', 'wait_buffer', 'prepare_next_step']
                        }
                        memory_mb = memory_consumption_mb()
                        stats = dict(memory_actor=memory_mb)
                        safe_put(self.report_queue, dict(times_actor_worker=timing_stats, stats=stats, policy_id=0), queue_name='report')
                        last_report = time.time()
