...
class DictObservationsWrapper(Wrapper):
    ...
    def step(self, action):
        obs, rew, done, info, raw_rew = self.env.step(action)
        return [dict(obs=o) for o in obs], rew, done, info, raw_rew
...
def cuda_envvars_for_policy(policy_id, process_type, gpu_mask=None):
    set_gpus_for_process(policy_id, 1, process_type, gpu_mask=gpu_mask)
    ...
    def cat(self, dict_of_tensor_arrays, macro_batch_size, use_pinned_memory, timing):
        ...
        if tensor_batch is not None:
            ...
            if old_batch_size != macro_batch_size:
                ...
                if False:
                    # This operation is currently deactivated since change in batchsize is inevitable due to memory.
                    # However, we do not want to re-allocate memory for batching so we deactivate this operation
                    # Copying data to cached the batch works desirably so we do not have to reallocate memory
