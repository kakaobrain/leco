...
def build_rnn_inputs(x, dones_cpu, rnn_states, T: int, actor_critic_share_weights=True, num_cores=1):
    """
    ...
    if actor_critic_share_weights:
        ...
        rnn_states_seq = rnn_states.index_select(0, rollout_starts)
        is_same_episode = (1 - is_new_episode.view(-1, 1)).index_select(0, rollout_starts)
        rnn_states_seq = rnn_states_seq * is_same_episode
    else:
        x_seq = []
        rnn_states_seq = []
        x_chunk = x.chunk(num_cores, dim=1)
        rnn_states_chunk = rnn_states.chunk(num_cores, dim=1)
        is_same_episode = (1 - is_new_episode.view(-1, 1)).index_select(0, rollout_starts)
        for i in range(num_cores):
            x_ = PackedSequence(x_chunk[i].index_select(0, select_inds), batch_sizes, sorted_indices)
            rnn_states_ = rnn_states_chunk[i].index_select(0, rollout_starts)
            rnn_states_ = rnn_states_ * is_same_episode
            x_seq.append(x_)
            rnn_states_seq.append(rnn_states_)

    return x_seq, rnn_states_seq, inverted_select_inds
