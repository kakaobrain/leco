...
class DummySampler(AlgorithmBase):
    ...
    def run(self):
        ...
        while not self.terminate.value:
                ...
                try:
                    ...
                    for msg in msgs:
                        ...
                        if 'crash' in msg:
                            ...
                            break
                        if 'completed' in msg: # for populate_level_cache
                            completed.append(msg['proc_idx'])
            ...
            for proc_idx in completed: # for populate_level_cache
                if proc_idx not in self.completed_processes:
                    p = self.processes[proc_idx]
                    log.info('Process %r completed...', p)
                    p.terminate()
                    self.completed_processes.append(proc_idx)
            completed = []

            for proc_idx, p in enumerate(self.processes):
                delay = time.time() - last_process_report[proc_idx] if proc_idx not in self.completed_processes else 0
                if delay > 600:
            ...
            for proc_idx, p in enumerate(self.processes):
                if not p.is_alive() and proc_idx not in self.completed_processes:
                    self.terminate.value = True
                    log.error('Process %r died! terminating...', p)

            if len(self.processes) == len(self.completed_processes): # for populate_level_cache
                self.terminate.value = True
